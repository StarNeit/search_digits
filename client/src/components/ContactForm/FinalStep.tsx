...
import { completeOnBoardingData } from '@services';
...
const FinalStep: React.FC<Props> = ({ onCloseModal }) => {
  const { setContactInformation, is_emergency, property_type, time } =
    useOnBoardingStore();
  ...
  const onSubmit = async (value: ContactForm) => {
    ...
    setContactInformation({ email, phone, address });

    try {
      const response = await completeOnBoardingData({
        is_emergency,
        property_type,
        time,
        email,
